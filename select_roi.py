...
ap.add_argument(
    "-i", "--dataset", required=True, type=str, help="path to input directory of images"
)
args = ap.parse_args()
...
for i, image in enumerate(images, start=1):
    ...
    im[:] = image
    df = df.append(
        pd.DataFrame(np.c_[im, bbox], columns=["file", "x1", "y1", "x2", "y2"]),
        ignore_index=True,
    )
...
print(
    "Output to: "
    + os.path.join(os.path.relpath(OUTDIR), f'{label.replace(" ","_")}_bbox.csv')
)
df.to_csv(
    os.path.join(OUTDIR, f'{label.replace(" ","_")}_bbox.csv'), index=False, header=True
)
